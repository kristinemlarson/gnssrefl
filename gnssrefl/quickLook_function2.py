...
def quickLook_function(station, year, doy, snr_type,f,e1,e2,minH,maxH,reqAmp,pele,satsel,PkNoise,
    ...
    if allGood == 0:
        ...
        if irefr == 1:
            snrD[:,1] = refr.corr_el_angles(snrD[:,1], quick_p, quick_T)
        ...
        #tt = 'GNSS-IR: ' + station.upper() + ' Freq:' + get_display_label(f) + ' Year/DOY:' + str(year) + ',' + str(doy) + ' elev: ' + str(e1) + '-' +  str(e2)
        # kristine uses fstrings!
        tt = f'GNSS-IR: {station.upper()} Freq: {get_display_label(f)} Year/DOY: {year:04d},{doy:03d} elev:{e1} -{e2}'
...
def goodbad(fname,station,year,doy,h1,h2,PkNoise,reqAmp,freq,e1,e2,hires_figs):
    ...
    #print(freq, get_display_label(freq))
    plt.title('quickLook Retrieval Metrics: ' + station + ' Freq:' + get_display_label(freq) + ydoy + ' elev:' + str(e1) + '-' + str(e2) ,fontsize=fs)
