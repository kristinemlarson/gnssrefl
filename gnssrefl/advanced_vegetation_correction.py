...
def norm_zero_vxyz(station, vxyz, remoutli, acc_rhdrift, baseperc, zphival, 
    ...
    qmat = vxyz[:, 12]      # Track avg azimuth
    ...
    for i in range(numtracks):
        ...
        print(f'avgAz {the_avg_az:5.1f} sat {thesat}  Offset {offs:6.3f} m  good {Ngood:4d}  bad {Nbad:4d}')
...
def apply_vegetation_model(station, vxyz, normmet, tracks, sgolnum, sgolply,
    ...
    for i in range(Nt):
        ...
        if Ntrack > min_number_points:
            ...
            if save_tracks:
                save_individual_track_data(station, int(year[ii][0]), thissat, this_avg_az,
...
def save_individual_track_data(station, track_year, sat_num, avg_az,
    ...
    track_file = f'{track_dir}/az{int(avg_az):03d}_sat{sat_num:02d}_{station}_{track_year}{freq_suffix}.txt'

    header_lines = [
        f"% Individual Track Data for Station {station}",
        f"% Satellite: {sat_num}, AvgAzimuth: {avg_az:.1f}, Year: {track_year}",
