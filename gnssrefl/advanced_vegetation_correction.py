...
def save_individual_track_data(station, track_year, sat_num, quad_num, 
                             ...
                             phase_orig, amp_lsp_orig, amp_ls_orig, rh_orig,
                             amp_lsp_smooth, amp_ls_smooth, rh_smooth,
    ...
    rh_orig : numpy.ndarray
        Original reflector height estimates (meters)
    ...
    rh_smooth : numpy.ndarray
        Smoothed reflector heights used for model lookup
    ...
    header_lines = [
        ...
        f"% Generated by Clara High Vegetation Model",
    ...
    with open(track_file, 'w') as f:
        ...
        for i in range(len(years)):
            f.write(f'{int(years[i]):4d} {int(doys[i]):3d} {hours[i]:6.2f} {mjds[i]:8.0f} {azimuths[i]:6.1f} '
                   ...
                   f'{amp_lsp_smooth[i]:12.6f} {amp_ls_smooth[i]:11.6f} {rh_smooth[i]:8.3f} '
