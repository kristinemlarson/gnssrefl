...
def save_individual_track_data(station, vxyz, subdir, fr):
    ...
    for sat_num, avg_az in unique_tracks:
        ...
        # Build (N, 17) array: cols 0-8 from vxyz, cols 9-16 NaN
        ...
        rows = np.column_stack([obs_cols, np.full((len(track_obs), 8), nan)])

        qp.write_track_file(track_dir, station, int(track_obs[0, 0]), sat_num,
                            avg_az, rows, fr,
                            extra_headers=[
