...
def save_individual_track_data(station, vxyz, extension, fr):
    ...
    station : str
        Station name
    vxyz : numpy array
        Track-level observations (N x 18)
    extension : str
    ...
    #   [16]track_id [17]track_epoch (-1 sentinels under -legacy T)
    sats = vxyz[:, 4]
    avg_azs = vxyz[:, 12]
    ...
    for k0, k1 in unique_tracks:
        if legacy:
            sat_num, avg_az = int(k0), k1
            mask = (sats == sat_num) & (avg_azs == avg_az)
            this_tid, this_ep = -1, -1
        else:
            this_tid, this_ep = int(k0), int(k1)
            mask = (track_id == this_tid) & (track_epoch == this_ep)
            if not mask.any():
                continue
            sat_num = int(sats[mask][0])
            avg_az = avg_azs[mask][0]
        track_obs = vxyz[mask]

        obs_cols = np.column_stack([
            track_obs[:, 0],   # year
            track_obs[:, 1],   # doy
            track_obs[:, 8],   # hour
            track_obs[:, 15],  # mjd
        ...
        nan_pad = np.full((len(track_obs), 8), nan)
        rows = np.column_stack([obs_cols, nan_pad])

        qp.write_track_file(track_dir, station, sat_num, avg_az, rows,
                            track_id=this_tid, track_epoch=this_ep,
                            extra_headers=[
                                "% Generated by Simple Vegetation Model (model 1)",
                                "% Columns 10-17 are NaN (model 1 does not compute per-track corrections)",
                            ])
